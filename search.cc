...
void search_in_file(const std::string& filename, const std::string& keyword) {
    ...
    while (true) {
        ...
        // std::cout << std::string_view(buffer.data(), bytes_read) << std::endl;
        count += search_in_chunk(std::string_view(buffer.data(), bytes_read), keyword);

        // Print current count on line
